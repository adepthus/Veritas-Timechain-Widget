# -*- coding: utf-8 -*-
"""
Timechain Desktop Widget v21.0.12a "Stabile Refactored"

Cel główny:
W pełni wielojęzyczna, dopracowana i zoptymalizowana aplikacja desktopowa.

ZMIANY W WERSJI v21.0.12a:
- VER: Zmiana numeracji na v21.0.12a.
- FIX: SettingsWindow otwiera się teraz na aktywnym monitorze i pozwala na skalowanie (fix dla High-DPI).
- REFACTOR: Całkowicie nowa logika generowania znaków wodnych.
  - Styl "Tiled": Generowany na powiększonym obszarze, aby uniknąć pustych rogów przy obrocie.
  - Styl "Arranged/Single": Obraca sam tekst (stempel), a nie całą warstwę, dzięki czemu układ (np. rogi ekranu) zostaje zachowany.
- OPT: Kod zgodny z PEP 8.
"""
...
# --- Metadata ---
APP_VERSION = "21.0.12a"
APP_CODENAME = "Stabile Refactored"
...
class TemplateEngine:
    ...
    def __init__(self, app: "TimechainApp"):
        ...
        self.special_placeholder_regex = re.compile("(%glyph%|%blockheight%|%hash%|@|'[^']*')")
    ...
    def render(self, config_override: Optional[Dict[str, Any]] = None) -> str:
        ...
        replacements = {'@': self._get_swatch_internet_time(), '%blockheight%': str(data.get("blockheight", "...")), '%hash%': data.get("hash_full", "...") if cfg.get("display_full_hash") else data.get("hash_short", "..."), '%glyph%': self._generate_glyph(cfg.get("glyph_seed", "")) if cfg.get("generate_glyphs") else ""}
        ...
        for i, line in enumerate(lines):
            ...
            if cfg.get("use_outline") and thickness > 0:
                if cfg.get("shadow_style") == "3d_offset":
                    ...
                    self.canvas.create_text(x_pos + ox, y_pos + oy, text=line, font=self._tk_font, fill=outline_color, anchor=anchor)
                else:
                    for dx, dy in [(dx, dy) for dx in range(-thickness, thickness + 1) for dy in range(-thickness, thickness + 1) if dx or dy]:
                        self.canvas.create_text(x_pos + dx, y_pos + dy, text=line, font=self._tk_font, fill=outline_color, anchor=anchor)
            self.canvas.create_text(x_pos, y_pos, text=line, font=self._tk_font, fill=text_color, anchor=anchor)
            y_pos += line_height
            
        self.canvas.config(width=canvas_width, height=canvas_height)
        try:
            _, x, y = self.master.geometry().split('+')
            self.master.geometry(f"{int(canvas_width)}x{int(canvas_height)}+{x}+{y}")
        except (IndexError, ValueError, tk.TclError):
            self.master.geometry(f"{int(canvas_width)}x{int(canvas_height)}+100+100")
            ...
            if _is_windows():
                ...
                for term in ['gnome-terminal', 'konsole', 'xterm', 'lxterminal', 'mate-terminal']:
                    if shutil.which(term):
                        subprocess.Popen([term, '--' if term == 'gnome-terminal' else '-e', 'bash', '-c', f'{command}; exec bash'] if term in ['gnome-terminal', 'konsole'] else [term, '-e', command])
                        return
...
class CaptureManager:
    ...
    def _add_watermark(self, image: Image.Image, arranged_positions: Optional[List[Tuple[float, float]]] = None) -> Image.Image:
        ...
        if style == "tiled":
            ...
            canvas_dim = hypotenuse + 200 # Margines bezpieczeństwa
...
class SettingsWindow:
    ...
    def _center_on_screen(self):
        ...
        try:
            ...
            # Znajdź monitor pod kursorem (prymitywne ale skuteczne w Tkinter bez zew. bibliotek)
            # Zakładamy domyślne centrowanie względem geometrii wirtualnego ekranu
            # Jednak lepsze jest wyśrodkowanie względem parenta lub kursora
            
            screen_width = self.master.winfo_screenwidth()
            screen_height = self.master.winfo_screenheight()
            
            # Jeśli mamy screeninfo, możemy być dokładniejsi
            if _OPTIONAL_DEPENDENCIES['screeninfo']:
                monitors = get_monitors()
                for m in monitors:
